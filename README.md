...
PostgreSQL stores companies, jobs, and scrape runs separately. Company names are unique, job URLs are unique, and repeated scraper runs update existing postings instead of creating duplicates.
...
Indexes support company, location, status, outreach status, follow-up date, posting date, scrape-date, and scrape-run queries.
